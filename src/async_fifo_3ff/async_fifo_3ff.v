module async_fifo_3ff #(
    ...
    reg [PTR_WIDTH-1:0] wr_ptr_b, rd_ptr_b;
    reg [PTR_WIDTH-1:0] wr_ptr_g_sync, rd_ptr_g_sync;
    wire [PTR_WIDTH-1:0] wr_ptr_g, rd_ptr_g;

    assign wr_ptr_g = (wr_ptr_b >> 1) ^ wr_ptr_b;
    assign rd_ptr_g = (rd_ptr_b >> 1) ^ rd_ptr_b;

    sync_3ff #(.WIDTH(DATA_WIDTH)) wr2rd_sync (
        .clk_dst(rd_clk),
        .rst_n(rd_rst_n),
        .din(wr_ptr_g),
    ...
    sync_3ff #(.WIDTH(DATA_WIDTH)) rd2wr_sync (
        .clk_dst(wr_clk),
        .rst_n(wr_rst_n),
        .din(rd_ptr_g),
